...
function validarConsulta($consulta) {
    $permitidos = ['SELECT'];
    $tokens = explode(' ', strtoupper(trim($consulta)));
    return in_array($tokens[0], $permitidos);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['usuario'])) {
        ...
        $stmt->bind_param("ss", $tipo_veiculo, $valor);
        $stmt->execute();
        $stmt->close();
        ...
        $stmt->execute();
        $stmt->close();
    } elseif (isset($_POST['criar_relatorio'])) {
        $nome = $_POST['nome'];
        $descricao = $_POST['descricao'];
        $consulta = $_POST['consulta'];

        if (validarConsulta($consulta)) {
            $stmt = $conn->prepare("INSERT INTO relatorios_personalizados (nome, descricao, consulta) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $nome, $descricao, $consulta);
            $stmt->execute();
            $stmt->close();
        } else {
...
<body>
    ...
    <form method="post">
        <input type="hidden" name="criar_relatorio" value="1">
        <label for="nome">Nome:</label>
        <input type="text" name="nome" required>
        <label for="descricao">Descrição:</label>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="visualizar_relatorio.php?id=<?php echo $relatorio['id']; ?>">Visualizar</a> | 
                    <a href="exportar_relatorio.php?id=<?php echo $relatorio['id']; ?>">Exportar como PDF</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
